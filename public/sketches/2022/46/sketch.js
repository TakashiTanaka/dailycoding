...
function draw() {
  ...
  I = calcInnerCenter(A, B, C);
  plotPointName('I', I);
  ellipse(I.x, I.y, 10);
  ellipse(I.x, I.y, calcIncircleRadius(A, B ,C) * 2);
}
...
const calcInnerCenter = (v1, v2, v3) => {
  const A = v1;
  const B = v2;
  const C = v3;
  const a = B.dist(C);
  const b = A.dist(C);
  const c = A.dist(B);
  return p5.Vector.mult(A, a).add(p5.Vector.mult(B, b)).add(p5.Vector.mult(C, c)).div(a + b + c);
}

const calcIncircleRadius = (v1, v2, v3) => {
  const A = v1;
  const B = v2;
  const C = v3;
  const a = B.dist(C);
